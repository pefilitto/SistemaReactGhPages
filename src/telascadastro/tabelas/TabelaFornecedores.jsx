...
import { buscarFornecedor, excluirFornecedor } from "../../redux/fornecedorSlicer";

export default function TabelaFornecedores(props) {
    const { conteudo, setFornecedorParaEdicao, setModoEdicao } = props;
    const [mostrarAlert, setMostrarAlert] = useState(false);
    const dispatch = useDispatch();
    const { estado, listaFornecedor, mensagem } = useSelector((state) => state.fornecedor)
    ...
    function verificaEstado(estado) {
        if (estado == 2) {
            ...
            setTimeout(() => setMostrarAlert(false), 6000)
        }
    ...
    function editarFornecedor(fornecedor) {
        console.log("Fornecedor para editar " + JSON.stringify(fornecedor))
